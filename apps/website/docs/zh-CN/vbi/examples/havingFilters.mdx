...
三层嵌套分组：OR(AND(销售额>50万, 利润>5万), AND(数量>100, 折扣均值<0.3))，模拟复杂业务筛选
...
散点图分析：按品类分组筛选高利润率(利润>销售额5%)且交易量>20笔的品类，定位优质业务
...
where+having联合筛选：先按where筛选办公用品品类，再按having筛选销售额>5万或利润>1万的省份
