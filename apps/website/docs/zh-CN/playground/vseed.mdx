...
const Demo = () => {
  ...
  const vseed: VSeed = {
    chartType: 'column',
    dataset: [
      { date: '2019', profit: 14, sales: 20, count: 50 },
      { date: '2020', profit: 30, sales: 60, count: 50 },
      { date: '2021', profit: 30, sales: 60, count: 50 },
      { date: '2022', profit: 54, sales: 100, count: 50 },
    ...
    barStyle: {
      dynamicFilter: {
        type: 'row-with-field',
        code: `
        return data.filter(item => item.profit / item.sales > 0.5).map(v => ({
          __row_index: v.__row_index,
          field: 'profit'
        }))
        `,
      },
      dynamicFilter: {
        type: 'chart-dynamic',
        code: `
        return data.filter(item => item.profit / item.sales > 0.5).map(v => _.pick(v, 'profit'))
        `,
      },
      barColor: 'lightpink',
      barColorOpacity: 0.8,
...
const SimpleVSeedRender = (props: { vseed: VSeed }) => {
  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      ...
      let spec = await builder.buildAsync()
