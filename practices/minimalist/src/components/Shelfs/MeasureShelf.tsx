...
export const MeasureShelf = ({ style }: { style?: React.CSSProperties }) => {
  ...
  useEffect(() => {
    ...
    return () => builder.measures.unobserve(updateMeasures);
  }, [builder]);
    ...
    try {
      ...
      if (data.type === 'measure') {
        builder.doc.transact(() => {
          builder.measures.addMeasure(data.name, (node) => {
    ...
    >
      ...
      {measures.map((measure) => (
        <Space.Compact key={`measure-shelf-${measure.field}`} style={{ width: '100%' }}>
          <Button shape="round" style={{ color: '#52c41a', flexGrow: 1, textAlign: 'left', borderRight: 0 }}>
            {measure.field}
          </Button>
          <Button
            shape="round"
