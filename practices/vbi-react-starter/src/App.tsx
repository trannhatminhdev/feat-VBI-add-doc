...
export function APP() {
  ...
  const [statusMessage, setStatusMessage] = useState(
    '先加载 demo 数据或上传 CSV，再用 starter components 组装图表。',
  );
  ...
  const hasAvailableFields =
    availableDimensions.length > 0 || availableMeasures.length > 0;
  const hasConfiguredFields =
    (dsl.dimensions?.length ?? 0) > 0 || (dsl.measures?.length ?? 0) > 0;
  ...
  const applyDataset = async (
    ...
    setStatusMessage(
      `已加载 ${data.length} 行数据。先在左侧添加维度和指标，再切换图表类型。`,
    );
  };
            ...
            >
              <div>
                ...
                <div style={{ color: '#9aa4d1', fontSize: 12, marginTop: 6 }}>
                  这个 demo 只使用 `@visactor/vbi-react/components` 来搭建核心编辑区，
                  用来验证 hooks + slim components 这条路线。
                </div>
              </div>
        ...
        main={
          hasConfiguredFields ? (
            <ChartRenderer
              ...
              debounce={150}
              loadingFallback={
                <EmptyState description="正在构建图表…" title="Building chart" />
              }
          ...
          >
            <div style={{ display: 'grid', gap: 6 }}>
              ...
              <div style={{ color: '#9aa4d1', fontSize: 12, maxWidth: 560 }}>
                `FieldPanel`、`ChartTypeSelector`、`ChartRenderer` 和
                `BuilderLayout`
            ...
            >
              <ChartTypeSelector
                builder={builder}
                style={{ color: '#e7ebff', minWidth: 180 }}
              />
              <button
                onClick={() => {
